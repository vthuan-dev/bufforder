...
router.put('/users/:id', verifyAdminToken, async (req, res) => {
  try {
    ...
    if (balance !== undefined) {
      ...
      console.log('[ADMIN PUT USER] newBalance/delta', { newBalance, delta, currentBalance: currentUser.balance, currentTotalDeposited: currentUser.totalDeposited });
      
      // Set new balance
      atomicUpdate.$set.balance = newBalance;
      
      // If balance increased, increase totalDeposited by the same amount
      if (delta > 0) {
        const newTotalDeposited = (currentUser.totalDeposited || 0) + delta;
        atomicUpdate.$set.totalDeposited = newTotalDeposited;
        console.log('[ADMIN PUT USER] Updating totalDeposited from', currentUser.totalDeposited, 'to', newTotalDeposited);
      }
    }
    ...
    // Double-check: if we updated balance but totalDeposited wasn't updated properly, fix it
    if (balance !== undefined) {
      const expectedDelta = Number(balance) - currentUser.balance;
      if (expectedDelta > 0) {
        const expectedTotalDeposited = (currentUser.totalDeposited || 0) + expectedDelta;
        if (user.totalDeposited !== expectedTotalDeposited) {
          console.log('[ADMIN PUT USER] FIXING totalDeposited - expected:', expectedTotalDeposited, 'actual:', user.totalDeposited);
          user.totalDeposited = expectedTotalDeposited;
        }
      }
    }
    
    // Update VIP level based on current totalDeposited
    user.updateVipLevel();
    await user.save();
