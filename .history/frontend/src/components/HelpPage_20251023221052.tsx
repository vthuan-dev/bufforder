...
export function HelpPage() {
  ...
  // Global message handlers - optimized for instant display
  const handleGlobalMessage = useCallback((event: any) => {
    const msg = event.detail;
    if (msg.threadId !== threadIdRef.current) return;
    
    console.log('[client] 📨 Instant message received:', msg);
    ...
    // Use functional update for immediate rendering
    setMessages(prev => {
      // Check if message already exists (by ID or by text+time for temp messages)
      const exists = prev.some(m => {
        // Match by real ID
        if (m.id === newMessage.id) return true;
        // Match temp messages by text and sender (replace with real message)
  ...
  const handleGlobalTyping = useCallback((event: any) => {
...
useEffect(() => {
  ...
  const handleSendMessage = useCallback((text?: string) => {
    ...
    try {
      ...
      console.error('[client] Failed to emit message:', err);
    }
  }, [inputMessage]);
