...
useEffect(() => {
  (async () => {
    try {
      ...
      try { threadId = localStorage.getItem('client:activeThreadId'); } catch {}
      if (threadId) {
        try {
      ...
      // Mark this thread as the active one for global notification suppression
    ...
    return () => {
      // Clear active thread marker when leaving HelpPage or unmounting
      try { localStorage.removeItem('client:activeThreadId'); } catch {}
      // DON'T remove the threadId from localStorage when just switching tabs
      // Only socketRef needs to be disconnected
